...
class Scene(SceneNode):
  ...
  def _touch_began(self, x, y, touch_id):
    ...
    if self.root_layer is not None:
      ...
      if hit_layer is not None:
        if hasattr(hit_layer, 'touch_began') and callable(
            hit_layer.touch_began):
  ...
  def _touch_moved(self, x, y, prev_x, prev_y, touch_id):
    ...
    if old_touch is not None:
      ...
      if touch.layer is not None:
        if hasattr(touch.layer, 'touch_moved') and callable(
            touch.layer.touch_moved):
  ...
  def _touch_ended(self, x, y, touch_id):
    ...
    if old_touch is not None:
      ...
      touch.layer = old_touch.layer
      if touch.layer is not None:
        if hasattr(touch.layer, 'touch_ended') and callable(
            touch.layer.touch_ended):
...
class ShapeNode(SpriteNode):
  ...
  def update_texture(self):
    ...
    w = max(
      1,
      math.ceil(path_bounds.w + abs(shadow_left) + abs(shadow_right)) + lw)
    h = max(
      1,
      math.ceil(path_bounds.h + abs(shadow_top) + abs(shadow_bottom)) + lw)

    with ui.ImageContext(w, h) as ctx:
      ui.concat_ctm(
        ui.Transform.translation(lw / 2 + max(0, shadow_left) - path_bounds.x,
                                 lw / 2 + max(0, shadow_top) - path_bounds.y))
      ui.set_color(self.fill_color)
      with ui.GState():
        if self.shadow:
          ui.set_shadow(shadow_color, shadow_offset_x, shadow_offset_y,
                        shadow_radius)
        self.path.fill()
